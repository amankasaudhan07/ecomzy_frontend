      ...
      <div className="md:w-[30%] w-full flex flex-col gap-8 justify-between">
        ...
        <div className="mt-20">
          ...
          <p className="font-[600] text-xl text-slate-700">
            Total Items: <span className="font-bold ml-2 text-green-600">{qty}</span>
          </p>
        </div>

        <div className="mb-20">
          <p className="text-slate-700 text-xl font-[600] mb-5 ">Total Amount: 
            <span className="font-bold ml-2 text-green-600">₹ {totalAmount}</span>
          </p>

          {
            cart.items.length>0&&(
              ...
              className="text-lg mb-5 w-full py-2.5 rounded-lg font-bold text-white bg-[#15803d]
              border-2 border-[#15803d] hover:bg-white hover:text-[#15803d] transition-all duration-300 ease-in">
                CheckOut Now
              </button>
              <button 
               onClick={()=>{if(confirm("are you sure,want to clear cart...?"))clearCart()}}
